...
规则是 8 月下旬为了让官网静态资源快一点，在面板上从模板一键创建的，配置全是模板默认值：匹配所有请求、Eligible for cache、Edge TTL 选「Ignore cache-control header and use this TTL」两小时、cache key 不定制。zone 同时开着 Smart Tiered Cache。
