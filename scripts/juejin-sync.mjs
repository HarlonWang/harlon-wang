...
function parseArgs(argv) {
    ...
    return { slug, force: flags.has('--force') };
}
...
async function syncOne(slug, { force }, ledger) {
    ...
    const result = await publish({ ...article, categoryId, tagIds, columnIds });
...
async function main() {
    ...
    for (const r of results) {
        const icon = { published: '✅', skip: '·', warn: '⚠️', error: '❌' }[r.status] ?? '?';
